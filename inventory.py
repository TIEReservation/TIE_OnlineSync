...
def load_combined_bookings(property: str, start_date: date, end_date: date) -> List[Dict]:
    try:
        ...
        for query_property in query_properties:
            online_response = supabase.table("online_reservations").select("*").in_("property", query_properties).lte("check_in", str(end_date)).gte("check_out", str(start_date))).gte("check_out", str(start_date)).execute()
