...
def create_bookings_table(bookings):
    ...
    columns = [
        ...
        "Total Tariff", "Advance Amount", "Balance Due", "Remarks"
    ]
    df_data = []
    for booking in bookings:
        ...
        df_data.append({
            ...
            "Total Tariff": booking.get("total_amount", 0.0),
            "Advance Amount": booking.get("advance_amount", 0.0),
...
@st.cache_data
def cached_load_reservations():
    """Cache the loading of reservations."""
    return load_reservations_from_supabase()

def show_daily_status():
    """Display Daily Status page with bookings."""
    ...
    bookings = cached_load_reservations()
    if not bookings:
    ...
    for prop in properties:
        with st.expander(f"{prop}"):
            ...
            # Filter bookings for the property
            prop_bookings = [b for b in bookings if b.get("property") == prop]
            st.info(f"Total bookings for {prop}: {len(prop_bookings)}")
            ...
            for day in month_dates:
                ...
                if daily_bookings:
                    ...
                    st.info("No bookings on this day.")
