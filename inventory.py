...
def compute_statistics(bookings: List[Dict], property: str, target_date: date, month_dates: List[date]) -> tuple:
    total_rooms = len([i for i in PROPERTY_INVENTORY.get(property, {}).get("all", []) if not i.startswith(("Day Use", "No Show"))])
    occupied = len(set(b["inventory_no"][0] for b in bookings if "inventory_no" in b))
    occupancy = (occupied / total_rooms * 100) if total_rooms > 0 else 0

    total_revenue = sum(b.get("receivable", 0) for b in bookings if b.get("is_primary"))
    total_advance = sum(b.get("advance", 0) for b in bookings if b.get("is_primary"))
    total_balance = sum(b.get("balance", 0) for b in bookings if b.get("is_primary"))

    return (occupancy, total_revenue, total_advance, total_balance)
        ...
        with st.expander(prop):
            ...
            for day in month_dates:
                ...
                if daily:
                    ...
                    occ, rev, adv, bal = compute_statistics(assigned, prop, day, month_dates)
                    ...
                    with c3:
                        ...
                        st.metric("Advance", f"₹{adv:,.2f}")
                        st.metric("Balance Due", f"₹{bal:,.2f}")
                else:
