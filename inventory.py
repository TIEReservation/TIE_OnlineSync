...
PROPERTY_INVENTORY = {
    "Le Poshe Beach view": {
        "all": [
            "101","102","201","202","203","204","301","302","303","304",
    ...
    "La Millionaire Resort": {
        "all": [
            "101","102","103","105","201","202","203","204","205","206","207","208",
            "301","302","303","304","305","306","307","308","401","402",
            "Day Use 1","Day Use 2","Day Use 3","Day Use 4","Day Use 5","No Show"
        ],
        "three_bedroom": ["203","204","205"]
    },
    "Le Poshe Luxury": {
        "all": [
            "101","102","201","202","203","204","205","301","302","303","304","305",
            "401","402","403","404","405","501","Day Use 1","Day Use 2","No Show"
        ],
        "three_bedroom": ["203","204","205"]
    },
    "Le Poshe Suite": {
    ...
    "La Tamara Luxury": {
        "all": [
            ...
            "301","302","303","304","305","306","401","402","403","404",
            "Day Use 1","Day Use 2","No Show"
        ],
...
def load_properties() -> List[str]:
    try:
        ...
        for r in res_direct:
            ...
            if p:
                props.add(property_mapping.get(p.strip(), p.strip()))
        for r in res_online:
            p = r.get("property")
            if p:
                props.add(property_mapping.get(p.strip(), p.strip()))
...
def normalize_booking(booking: Dict, is_online: bool) -> Dict | None:
    ...
    prop = property_mapping.get(
        sanitize_string(booking.get("property", booking.get("property_name"))),
        sanitize_string(booking.get("property_name")),
    )

    total_tariff = (
        safe_float(booking.get("total_amount_with_services", booking.get("booking_amount", 0.0)))
        or safe_float(booking.get("total_tariff", 0.0))
    )
    advance = (
        safe_float(booking.get("total_payment_made", 0.0))
        or safe_float(booking.get("advance_amount", 0.0))
    )
    balance = (
        safe_float(booking.get("balance_due", 0.0))
        or safe_float(booking.get("balance_amount", 0.0))
    )
...
# ----------------------------------------------------------------------
# Load bookings that overlap the month (cross-month safe)
# ----------------------------------------------------------------------
def load_combined_bookings(property: str, start_date: date, end_date: date) -> List[Dict]:
    try:
        ...
        logging.info(f"Overlap query for {property}: {start_date} → {end_date}")
        ...
        for qp in query_props:
            ...
            online.extend(
                [b for b in (resp.data or []) if (norm := normalize_booking(b, True))]
            )
            ...
            direct.extend(
                [b for b in (resp.data or []) if (norm := normalize_booking(b, False))]
            )
...
# ----------------------------------------------------------------------
# Inventory assignment – safe access
# ----------------------------------------------------------------------
def assign_inventory_numbers(daily: List[Dict], prop: str) -> tuple[List[Dict], List[Dict]]:
    ...
    for b in daily:
        ...
        total_pax = safe_int(b.get("total_pax", 0))
        receivable = safe_float(b.get("receivable", 0.0))
        days = safe_int(b.get("days", 1)) or 1

        base_pax = total_pax // len(rooms) if rooms else 0
        rem_pax = total_pax % len(rooms) if rooms else 0
        per_night = receivable / len(rooms) / days if rooms and days > 0 else 0.0

        if len(rooms) == 1:
            bc = b.copy()
            bc["inventory_no"] = rooms
            bc["per_night"] = per_night
            bc["is_primary"] = True
            assigned.append(bc)
        else:
    ...
    for b in assigned:
        ...
        for inv in inv_no:
            ...
            if b.get("is_primary") and first_day:
                row.update({
                    f: f"{b.get(f.lower().replace(' ','_'),0):.2f}" if f not in ["Advance Mop"] else b.get(f.lower().replace(' ','_'),"")
                    for f in fin
                })
...
def compute_mop_report(daily_assigned: List[Dict], target: date) -> pd.DataFrame:
    ...
    for b in daily_assigned:
        ...
        for std, vars_ in mop_mapping.items():
            if adv_mop in vars_:
                data[std] += adv
                total += adv
                if std == "Cash":
    ...
    return pd.DataFrame(
        [{"MOP": k, "Amount": f"{v:.2f}"} for k, v in data.items()],
        columns=["MOP", "Amount"]
    )
...
def compute_statistics(
    bookings: List[Dict], prop: str, target: date, month_dates: List[date]
    ...
    for b in assigned:
        ...
        mob = next((std for std, vars_ in mob_mapping.items() if raw.upper() in [v.upper() for v in vars_]), "Booking")
        ...
        dtd_tot_rooms += rooms; dtd_tot_val += val; dtd_tot_pax += b["total_pax"]
        dtd_tot_gst += b["gst"] if b.get("is_primary") and b.get("target_date")==date.fromisoformat(b["check_in"]) else 0.0
        dtd_tot_comm += com
    ...
    dtd["Total"] = {
        "rooms":dtd_tot_rooms,"value":dtd_tot_val,
    ...
    for day in month_dates:
        ...
        for b in ass:
            ...
            mtd_tot_rooms += rooms; mtd_tot_val += val; mtd_tot_pax += b["total_pax"]
            mtd_tot_gst += b["gst"] if b.get("is_primary") and b.get("target_date")==date.fromisoformat(b["check_in"]) else 0.0
            mtd_tot_comm += com
    ...
    mtd["Total"] = {
        "rooms":mtd_tot_rooms,"value":mtd_tot_val,
        "arr":mtd_tot_val/mtd_tot_rooms if mtd_tot_rooms else 0.0,
        "comm":mtd_tot_comm
    }
    mtd_df = pd.DataFrame([
        {"MOB":m, "M.T.D Rooms":d["rooms"], "M.T.D Value":f"{d['value']:.2f}",
         "M.T.D ARR":f"{d['arr']:.2f}", "M.T.D Comm":f"{d['comm']:.2f}"}
        for m,d in mtd.items()
    ])
        ...
        with st.expander(prop):
            ...
            for day in month_dates:
                ...
                if daily:
                    ...
                    st.markdown(
                        f'<div class="custom-scrollable-table">{inv_df.to_html(escape=False, index=False)}</div>',
