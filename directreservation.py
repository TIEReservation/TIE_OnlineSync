...
def show_new_reservation_form():
    ...
    with st.form(key="new_reservation_form"):
        ...
        with col5:
            ...
            total_tariff = st.number_input("Total Tariff", min_value=0.0, step=100.0, value=st.session_state.total_tariff, key="total_tariff_new", on_change=update_balance)
            advance_amount = st.number_input("Advance Amount", min_value=0.0, step=100.0, value=st.session_state.advance_amount, key="advance_amount_new", on_change=update_balance)
        ...
        if st.form_submit_button("📥 Submit Reservation"):
            if not guest_name or not guest_phone or not check_in or not check_out:
                ...
                new_reservation = {
                    ...
                    "balance_amount": st.session_state.balance_amount,
