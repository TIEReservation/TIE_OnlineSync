...
def show_analytics():
    ...
    if start_date and end_date:
        filtered_df = filtered_df[(pd.to_datetime(filtered_df["Check In"]) >= pd.to_datetime(start_date)) & (pd.to_datetime(filtered_df["Check In"]) <= pd.to_datetime(end_date))]
    elif start_date:
        filtered_df = filtered_df[pd.to_datetime(filtered_df["Check In"]) >= pd.to_datetime(start_date)]
    elif end_date:
        filtered_df = filtered_df[pd.to_datetime(filtered_df["Check In"]) <= pd.to_datetime(end_date)]
...
def display_filtered_analysis(df, start_date, end_date, view_mode=True):
    """Filter and return the DataFrame based on date range."""
    filtered_df = df.copy()
    if start_date and end_date:
        filtered_df = filtered_df[(pd.to_datetime(filtered_df["Check In"]) >= pd.to_datetime(start_date)) & (pd.to_datetime(filtered_df["Check In"]) <= pd.to_datetime(end_date))]
    elif start_date:
