...
This updates the `show_reservations` function to fix the `KeyError` by replacing `"booking_status"` with `"status"` in the `display_columns` list, assuming the `reservations` table uses `status` (based on typical Supabase schema conventions). If the actual column name differs, you'll need to adjust it after verifying the table schema.

<xaiArtifact artifact_id="a62e647a-84eb-42cf-b5f4-e1098c05a1c5" artifact_version_id="eabbd36c-1325-42a4-897f-6467cceb67e7" title="directreservation.py" contentType="text/python">
```python
import streamlit as st
import pandas as pd
...
def show_reservations():
    ...
    if filter_status != "All":
        filtered_df = filtered_df[filtered_df["status"] == filter_status]  # Changed: Use "status" instead of "booking_status"
    ...
    if filtered_df.empty:
        ...
        display_columns = [
            ...
            "room_no", "room_type", "status", "payment_status", "total_tariff",  # Changed: "booking_status" to "status"
