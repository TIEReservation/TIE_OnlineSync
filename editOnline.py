...
def show_edit_online_reservations(selected_booking_id=None):
    ...
    if edit_index is not None:
        ...
        with st.form("edit_online_reservation_form"):
            ...
            property_options = sorted(list(property_map.keys()))
            property = st.selectbox("Property", property_options, index=property_options.index(reservation.get("property", "")) if reservation.get("property", "") in property_options else 0)
            ...
            # Room Type and Room No with all options, including No Show and Day Use
            room_types = sorted(list(property_map.get(property, {}).keys()))
            ...
            room_numbers = property_map.get(property, {}).get(room_type, [])
