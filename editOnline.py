...
from inventory import PROPERTY_INVENTORY  # Import for room dropdown
...
def load_properties():
    ...
    try:
        ...
        properties = sorted(set([r['property_name'] for r in res_direct if r['property_name']]))
        return properties
    except Exception as e:
...
def show_edit_online_reservations(selected_booking_id=None):
    ...
    if st.session_state.online_edit_mode and st.session_state.online_edit_index is not None:
        ...
        with st.form("edit_online_form"):
            ...
            with col2:
                room_type = st.text_input("Room Type", value=reservation["room_type"])
